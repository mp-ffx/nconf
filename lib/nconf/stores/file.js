...
var File = exports.File = function (options) {
  ...
  if (this.secure) {
    this.secure.alg = this.secure.alg || 'aes-256-gcm';

    if (!this.secure.key) {
      throw new Error('secure.key option is required');
    }

    if (!this.secure.iv) {
      throw new Error('secure.iv option is required');
    }
...
File.prototype.parse = function (contents) {
  ...
  if (this.secure) {
    ...
    parsed = Object.keys(parsed).reduce(function (acc, key) {
      ...
      var decipher = crypto.createDecipheriv(value.alg, self.secure.key, self.secure.iv);
      if (value.iv) {
        // For backward compatibility, use createDecipheriv only if there is iv stored in file
        decipher = crypto.createDecipheriv(value.alg, self.secure.key, Buffer.from(value.iv, 'hex'));
      } else {
        outdated = true;
      }
      if (value.authTag) {
